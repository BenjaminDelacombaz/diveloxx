...
export class CanUpdateDiveSiteGuard implements CanActivate {
  constructor(
          ...
          return this.diverService.getDiverByUid(user.uid)
            .pipe(
              switchMap((currentDiver: Diver) => {
                ...
                return this.diveSiteService.getDiveSite(next.paramMap.get('id'))
                  .pipe(
                    map((diveSite: DiveSite) => {
                      if (!currentDiver.canUpdate(diveSite)) {
                        // Display error toast
                        this.toastController.create({
